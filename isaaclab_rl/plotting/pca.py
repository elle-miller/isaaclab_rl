...
def plot_joint_pca(z, z_hat, c=None, ep_length=250, n_components=None, plot_style='seaborn-whitegrid', stack=False, scale=False):
    """
    ...
    ax.scatter(z_hat_pca[:, 0], z_hat_pca[:, 1], c=c, label='Forward Model Output (z_hat)', cmap="plasma",
               alpha=0.7, s=50, marker='x')
    
    # Add connecting lines between corresponding points
    for i in range(len(z_pca)):
        ax.plot([z_pca[i, 0], z_hat_pca[i, 0]], [z_pca[i, 1], z_hat_pca[i, 1]], 
    ...
    if plot_variance:
        ...
        for threshold in [0.8, 0.9, 0.95]:
            ...
            if any(cumulative_variance >= threshold):
                ...
                axs[1].text(len(components), threshold, 
                        f' {threshold:.0%}', 
                        verticalalignment='bottom')
                axs[1].text(component_idx + 1, threshold, 
                        f' {component_idx + 1} components', 
                        verticalalignment='bottom')
    ...
    if z_pca.shape[1] >= 3:  # If we have at least 3 components
        ...
        ax_3d.scatter(z_pca[:, 0], z_pca[:, 1], z_pca[:, 2], 
                     label='Original Trajectory (z)', alpha=0.7, s=50)
        ax_3d.scatter(z_hat_pca[:, 0], z_hat_pca[:, 1], z_hat_pca[:, 2], 
                     label='Forward Model Output (z_hat)', alpha=0.7, s=50, marker='x')
        
        # Connect with lines
        for i in range(len(z_pca)):
            ax_3d.plot([z_pca[i, 0], z_hat_pca[i, 0]], 
